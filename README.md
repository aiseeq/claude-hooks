...
```
 CLAUDE-HOOKS   ~/work/claude-hooks
 main ●3 · Opus 5·high · ctx ▓▓▓▓░░░░░░ 47% · +412 −87
```
...
| жёлтый | Claude спрашивает разрешение или ждёт ответа |
| красный | контекст заполнен больше чем на 85% — скоро сожмётся история |

Вторая строка собирается из того, что прислал Claude Code, и пустых разделителей не оставляет:

| Элемент | Значение |
|---|---|
| `main ●3 ↑2 ↓1` | ветка, изменённые файлы, коммиты впереди и позади remote |
| `Opus 5·high` | модель и уровень рассуждений |
...
Konsole собирает заголовок по шаблону (`%d : %n` по умолчанию) и escape-последовательность OSC при этом игнорирует, поэтому шаблон заменяется целиком через `org.kde.konsole.Session.setTabTitleFormat`. Сессия находится по переменным `KONSOLE_DBUS_SERVICE` и `KONSOLE_DBUS_SESSION`, которые Konsole передаёт своим процессам. В остальных терминалах заголовок ставится обычной последовательностью OSC 0.
